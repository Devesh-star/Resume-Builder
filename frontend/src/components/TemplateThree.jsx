...
const TemplateThree = ({ resumeData = {}, containerWidth }) => {
  ...
  const formatDescription = (desc) => {
    ...
    return desc.split('\n').filter(line => line.trim().length > 0).map((line, i) => (
      <li key={i} className="ml-4 list-disc text-sm text-gray-700 leading-relaxed mb-1 marker:text-[#D4577A]">
        {line.replace(/^-/, '').trim()}
      </li>
    ));
  };
  ...
  return (
    ...
    >
      {/* Left Column (Accent) */}
      <div className="w-[35%] bg-[#FAF5F0] p-8 border-r border-[#E88FA2]/20 shrink-0">
        <div className="mb-8">
          <div className="w-24 h-24 bg-[#D4577A] text-white rounded-full flex items-center justify-center text-4xl font-bold mb-4 shadow-lg mx-auto">
          ...
          <p className="text-[#D4577A] font-bold text-center mt-2 text-sm uppercase tracking-wider">
        ...
        <div className="space-y-8">
          {/* Contact */}
          <div>
            <h2 className="text-sm font-bold text-gray-900 uppercase tracking-widest mb-4 border-b border-[#E88FA2]/30 pb-2">
              Contact
            </h2>
          ...
          {skills.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {skills.map((s, i) => (
                  <span key={i} className="px-3 py-1 bg-white border border-[#E88FA2]/50 text-[#D4577A] text-xs font-bold rounded-full shadow-sm">
                    {s.name}
                  </span>
                ))}
              </div>
        ...
        {profileInfo.summary && (
          <div className="mb-8">
            <h2 className="text-lg font-bold text-gray-900 uppercase tracking-widest mb-3 flex items-center">
              <span className="w-8 h-[2px] bg-[#D4577A] mr-3"></span>
              Profile
            </h2>
            ...
            <div className="space-y-6 border-l-2 border-gray-100 ml-2 pl-6">
              {workExperience.map((exp, i) => (
                <div key={i} className="relative">
                  <div className="absolute w-3 h-3 bg-[#D4577A] rounded-full -left-[31px] top-1.5 ring-4 ring-white"></div>
                  <h3 className="font-bold text-gray-900 text-base">{exp.role}</h3>
                  <div className="text-[#D4577A] text-sm font-bold mb-2">
              ...
              {projects.map((proj, i) => (
                <div key={i} className="relative">
                  ...
                  <div className="flex gap-3 mt-1 mb-2">
                    {proj.liveDemo && (
                      <a href={proj.liveDemo} className="text-xs font-bold text-[#D4577A] hover:underline">Live Demo</a>
                    )}
                    {proj.github && (
                      <a href={proj.github} className="text-xs font-bold text-[#D4577A] hover:underline">GitHub</a>
                    )}
                  </div>
